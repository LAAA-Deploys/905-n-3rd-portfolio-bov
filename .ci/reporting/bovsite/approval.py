...
#: Any person may approve; the record names whoever ACTUALLY approved and that
#: name is published (Glen, 09/21/2026: no roster gate). This map is only a
#: convenience so `bov approve` can DERIVE the approver from the authenticated
#: `gh` identity when `--approved-by` is omitted. It is not an allowlist: a
#: login missing from it passes `--approved-by "<own full name>"` instead.
APPROVERS = {
...
def approver_errors(name: object) -> list[str]:
    ...
    if not isinstance(name, str) or len(name.strip()) < 2:
        return ["approver must name the person who actually approved, got " + repr(name)]
    return []
...
def resolve_approver(explicit: str | None = None) -> str:
    ...
    if login not in APPROVERS:
        raise ApprovalError(
            f"GitHub login {login!r} has no name on file, so the approver could not be "
            "derived. The approver is PUBLISHED in the site's approval record, so pass "
            "--approved-by \"<your own full name>\". Do not approve under another "
            "person's name.")
...
def build_deal_record(
    ...
    *,
    ...
    if approver_errors(approver):
        raise ApprovalError(
            approver_errors(approver)[0] + ". The approver is PUBLISHED in the site's "
...
def check_deal_record(workspace: DealWorkspace, record_path: Path | None = None) -> tuple[list[str], list[str], dict | None]:
    ...
    errors.extend("approval " + e for e in approver_errors(record["approver"]))
