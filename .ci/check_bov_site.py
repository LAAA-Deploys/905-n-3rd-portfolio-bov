...
def main(site):
    ...
    for page in pages:
        ...
        # The hub of a portfolio carries the resume once for every route.
        if len(property_slugs) > 1 and page == site / "index.html":
            for section in ("track-record", "marketing", "contact"):
                if section not in found:
                    fail(f"{rel}: the portfolio hub is missing its {section} section.")
        if is_property_page:
            mandatory = {
            ...
            # A portfolio route leaves the resume to the hub (Glen, 2026-09-22).
            if len(property_slugs) > 1:
                mandatory -= {"track-record", "marketing"}
                for section in ("track-record", "marketing", "active-listings"):
                    if section in found:
                        fail(f"{rel}: a portfolio property route repeats the hub's {section} section.")
